...
def build_distill_basis(backbone: nn.Module, model_name: str,
                        ...
                        seed: int = 0,
                        shrink: float = 0.0,
                        W_shrink: Optional[torch.Tensor] = None
    ...
    # ---- optional shrinkage toward the kernel target (the ONLY principled
    # "kernel + distill"): Sigma_shrunk = (1-lambda) Sigma~ + lambda sigma^2 WW^T.
    # This does NOT improve distill's orientation -- distill already saw the
    # full covariance. It trades a little orientation back for VARIANCE
    # reduction when the synthetic sample is small/noisy (few batches). The
    # kernel target sigma^2 WW^T is scale-matched to Sigma~ via trace. As the
    # synthesis budget grows the optimal lambda -> 0 (pure distill). ----
    shrink_info = {'shrink': float(shrink)}
    if shrink > 0.0 and W_shrink is not None:
        Wd = W_shrink.to(Sigma.device, Sigma.dtype)
        WWt = Wd @ Wd.T                                     # [C_out, C_out]
        # scale the kernel target to match Sigma~'s total variance (trace),
        # so lambda mixes orientation, not magnitude.
        tr_sigma = torch.trace(Sigma).clamp(min=1e-12)
        tr_wwt = torch.trace(WWt).clamp(min=1e-12)
        WWt_scaled = WWt * (tr_sigma / tr_wwt)
        Sigma = (1.0 - shrink) * Sigma + shrink * WWt_scaled
        Sigma = 0.5 * (Sigma + Sigma.T)
        shrink_info['shrink_target'] = 'sigma^2 WW^T (trace-matched)'
...
def build_from_weights(model_name: str, target_layer_name: str, D: int,
                       ...
                       distill_seed: int = 0, distill_shrink: float = 0.0,
    ...
    distill_shrink : in [0,1]. For basis='distill' only. Shrinks the estimated
                     covariance toward sigma^2 WW^T (the kernel target) -- the
                     ONLY principled "kernel + distill". 0 == pure distill
                     (default, best when synthesis budget is adequate); >0
                     stabilizes a noisy small-sample Sigma~ at the cost of
                     pulling orientation back toward white.
    device         : 'auto' (CUDA if available -- default), 'cuda', or 'cpu'.
    """
    if device == 'auto':
    ...
    if basis == 'kernel':
        ...
        if distill_shrink > 0.0:
            print(f"  Distill shrinkage: lambda={distill_shrink:.2f} toward "
                  f"sigma^2 WW^T (trace-matched kernel target).")
        V, mu_d, dinfo = build_distill_basis(
            ...
            seed=distill_seed, shrink=distill_shrink, W_shrink=W)
        sv = None
