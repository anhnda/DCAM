...
def load_dcam_from_result(result_path: str, device: str) -> Tuple[DCAM, dict]:
    """Reconstruct a DCAM module from a run_dcam_full.py result .pkl.

    The training script saves:
        {'Pi': [D, C] tensor, 'Pi0': [D, C] tensor, 'config': {...}, ...}

    We rebuild the DCAM module from `config` and copy Pi into it. We do NOT
    rely on Pi0 here -- the saved Pi already encodes the trained state, and
    init_from_anchor would overwrite it.
    """
    ...
    if Pi.shape != (D, C):
        raise ValueError(f"Pi shape {tuple(Pi.shape)} != config (D, C) = ({D}, {C})")
...
def main():
    ...
    # Sanity-check: backbone/layer in the checkpoint match the CLI args.
    if dcam_cfg.get('model') != args.model:
        print(f"  WARNING: checkpoint backbone '{dcam_cfg.get('model')}' "
              f"!= --model '{args.model}'. Using --model.")
    ckpt_layer = dcam_cfg.get('target_layer')
    if ckpt_layer is not None and ckpt_layer != target_layer:
        print(f"  WARNING: checkpoint target_layer '{ckpt_layer}' "
              f"!= '{target_layer}'. Using '{target_layer}'.")
