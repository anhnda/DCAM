...
channel selection is unchanged under input perturbation, where
    L_f      = ||W_e||_2                          (encoder operator norm)
    gamma_K  = s_(K) - s_(K+1)                    (K-th vs (K+1)-th score gap)
    s_d      = sum_{i,j} ReLU(W_e A + b_e)_{d,i,j}

Claims tested here (single trained SAE):
  (A) Top-K threshold:
        if eps < eps*(x) := gamma_K(x) / (2 sqrt(HW) L_f L_Phi),
        the support is unchanged.
  (B) Top-K Jaccard scales with eps; per-image gamma_K(x) predicts
      per-image stability (boundary churn).

Claim C of the Remark (L_gc widens gamma_K, improving Top-K stability)
requires a separate two-checkpoint ablation -- see compare_gc_ablation.py.

Predictions:
  (P1) Top-K Jaccard decreases monotonically in eps.
  (P2) Sym-diff ratio |S Δ S'| / K stays small even when support flips
       (boundary-only churn).
  (P3) Per-image gamma_K(x) predicts per-image Top-K Jaccard
       (positive Spearman across the test set).
  (P4) eps*(x) = gamma_K(x) / (2 sqrt(HW) L_f L_Phi) rank-orders
       per-image stability (monotone rescaling of P3).
...
# ==========================================================================
# Core: Top-K support and gamma_K from encoder pre-activations
# ==========================================================================
...
def compute_topk_support(csae_model: MultiChannelConvSAE,
                         A: torch.Tensor,
                         K: int):
    """Compute Top-K support set, importance scores, and gamma_K.

    Args
    ----
    csae_model : trained ConvSAE.
    A          : [1, C, H, W] activation tensor from the target layer.
    K          : Top-K parameter.

    Returns
    -------
    support    : bool [D], True for selected (top-K) channels.
    scores     : [D] importance scores s_d.
    gamma_K    : float, s_(K) - s_(K+1)  (np.inf if K == D).
    z_pre      : [1, D, H, W] post-ReLU features before Top-K masking.
    """
    z_pre = csae_model.encoder(A)
    z_pre = F.relu(z_pre)                              # [1, D, H, W]
    scores = z_pre[0].sum(dim=(1, 2))                  # [D]
    D = scores.shape[0]
...
def estimate_L_f(csae_model: MultiChannelConvSAE) -> float:
    """Spectral norm of the encoder weight (a 1x1 conv unfolds to a
    [D, C] matrix)."""
    W = csae_model.encoder.weight.detach()
    if W.dim() == 4:                                   # [D, C, kh, kw]
...
def estimate_L_phi(extractor: MultiModelActivationExtractor,
    ...
    """Finite-difference estimate of L_Phi = sup ||dA||_F / ||dx||.

    Returns the 95th percentile across all (image, probe) pairs.
    A small probe magnitude is used to stay in the locally linear regime.
    """
    ratios = []
...
def run_evaluation(args):
    ...
    extractor = MultiModelActivationExtractor(
        ...
        device=device,
        cumulative_threshold=args.tau,  # unused here; kept for API parity
    )
    ...
    denom = 2.0 * np.sqrt(H * W) * composite_L
    print(f"  L_f * L_Phi                = {composite_L:.4f}")
    print(f"  Prop 5.6 threshold prefactor: "
          f"eps*(x) = gamma_K(x) / {denom:.4f}")

    # ---------------- Main loop ----------------
    print("\n" + "=" * 72)
    print(f"Top-K stability over {len(eval_dataset)} images")
    print(f"K = {K}, epsilons = {args.epsilons}, trials/eps = "
          f"{args.trials_per_eps}")
    print("=" * 72)
    ...
    for img_idx, (image, label) in enumerate(tqdm(eval_loader, desc="Images")):
        ...
        # Per-image safe radius (Prop 5.6)
        if composite_L > 1e-12:
            eps_star = gamma_K / (2.0 * np.sqrt(H * W) * composite_L)
        else:
            eps_star = float('inf')

        for eps in args.epsilons:
            j_trials, flipped_trials, sd_trials = [], [], []

            for _ in range(args.trials_per_eps):
                if eps == 0.0:
                    image_pert = image
                else:
                ...
                support_pert, _, _, _ = compute_topk_support(csae_model, A_pert, K)
    ...
    print("=" * 72)
    sds_header = "|SΔS'|/K"
    print(f"{'eps':>10} {'Jaccard':>9} {'flip':>8} "
    ...
    for eps in args.epsilons:
        ...
        # P4: rho(eps_star, Jaccard) -- monotone in gamma_K, so usually equals rho_g
        ...
        print(f"{eps:>10.4g} {j_mean:>9.4f} {flip:>8.4f} "
...
# ==========================================================================
# Plots (6-panel diagnostic figure)
# ==========================================================================
...
def make_plots(summary, save_path, model_name):
    ...
    flips = [summary['per_epsilon'][eps_str_map[e]]['flip_rate']
             for e in epsilons]
    ax.plot(epsilons, means, 'o-', color='C0', linewidth=2,
            label='Top-K Jaccard')
    ax2 = ax.twinx()
    ax2.plot(epsilons, flips, 's--', color='C3', linewidth=2,
             label='Flip rate')
    ax.set_xscale('log')
    ...
    # ---- (e) eps* distribution vs tested epsilons ----
    ax = axes[1, 1]
    rs = summary['per_image'][eps_str_map[epsilons[0]]]
    ...
    if eps_stars:
        ...
        for e in epsilons:
            if e > 0:
                ax.axvline(np.log10(e), color='gray', alpha=0.4, linestyle=':')
        smallest_nonzero = next((e for e in epsilons if e > 0), None)
        if smallest_nonzero is not None:
            ax.axvline(np.log10(smallest_nonzero), color='red', alpha=0.7,
                       linestyle='--',
                       label=f'min tested $\\epsilon$ = {smallest_nonzero:.3g}')
    ax.set_xlabel(r'$\log_{10} \epsilon^*(x)$')
    ax.set_ylabel('Count')
    ax.set_title(r'(e) $\epsilon^*(x)$ vs tested $\epsilon$')
    ax.grid(True, alpha=0.3)
    ...
    ax.set_title(r'(f) Do $\gamma_K$ and $\epsilon^*$ rank-order stability?')
...
def write_report(summary, save_path, model_name):
    ...
    lines.append(f"  - L_f * L_Phi = {cfg['L_f_times_L_phi']:.4g}: composite")
    lines.append(f"    Lipschitz constant entering the Prop 5.6 threshold.")
    lines.append(f"  - gamma_K(x) (per-image gap between K-th and (K+1)-th")
    lines.append(f"    importance scores):")
    ...
    lines.append(f"    Small gamma_K => K-th and (K+1)-th channels are close")
    lines.append(f"    in importance and a small perturbation can swap them.")
    ...
    lines.append("                 small even when flip rate is high => boundary")
    lines.append("                 churn only (good).")
    lines.append("  - <eps*      : fraction of samples below Prop 5.6 threshold.")
    lines.append("  - rho(γ,J)   : P3 -- positive means larger gap predicts more")
    lines.append("                 stable support (expected).")
    lines.append("  - rho(e*,J)  : P4 -- per-image threshold rank-orders stability")
    lines.append("                 (monotone rescaling of P3, usually identical).")
    lines.append("")
    lines.append("What to look for:")
    lines.append("  P1: Jaccard decreases monotonically in eps.")
    lines.append("  P2: sd_ratio stays small even when flip rate -> 1.")
    lines.append("  P3: rho(gamma_K, Jaccard) > 0 across all eps.")
    lines.append("  P4: rho(eps*, Jaccard) > 0, magnitude similar to P3.")
    lines.append("")
    lines.append("Note: this script verifies Claims A and B of Prop 5.6.")
    lines.append("Claim C of the Remark (L_gc widens gamma_K, improving Top-K")
    lines.append("stability) requires compare_gc_ablation.py, which contrasts")
    lines.append("two checkpoints trained with lambda_gc = 0 vs lambda_gc > 0.")
...
def main():
    ...
    parser.add_argument('--tau', type=float, default=0.95,
                        help='(Unused here; kept for extractor API parity.)')
